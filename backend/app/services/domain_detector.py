...
DOMAIN_RULES: Dict[str, Dict[str, Set[str]]] = {
    ...
    "AE": {
        ...
        "weak": {
            ...
            "AE_END_TIME_RAW", "AE_FORM_SEQ", "AE_SEQ_CRF", "AE_SEQ_CRF".replace("_CRF", "_CRf"),
            "AE_COMMENT",
        },
    },
    "LB": {
...
def _score_domains(columns: Set[str]) -> Tuple[Optional[str], int, List[str], Dict[str, int]]:
    scores: Dict[str, int] = {}
...
def detect_domain_from_columns(columns: Iterable[str]) -> Dict[str, object]:
...
def detect_domain_from_dataframe(df: pd.DataFrame) -> Dict[str, object]:
