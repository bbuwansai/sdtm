...
def execute_sdtm_only(job_id: str, domain: str, upload_path: Path, reviewed_human_path: Path) -> None:
    ...
    if domain == "DM":
        ...
        shutil.copy2(domain_dir / "Layer1" / "dm_cleaned_output.csv", sdtm_dir / "dm_cleaned_output.csv")
        shutil.copy2(domain_dir / "Layer1" / "dm_sdtm_standardizable_issues.csv", sdtm_dir / "dm_sdtm_standardizable_issues.csv")
        shutil.copy2(reviewed_human_path, sdtm_dir / "dm_human_review_issues.csv")

        store.append(job_id, "Preparing SDTM inputs using Layer 1 outputs, reviewed human issue log, and generated spec artifacts.", step="SDTM generation")
        run_command(["python", "sdtm_v4.py"], sdtm_dir, job_id, "SDTM generation")

        sdtm_out = sdtm_dir / "sdtm_outputs_v4"
